...
public class Main {
  ...
  public static int[] solve(int[] arr) {
    ...
    // tranverse left to right
    for (int i = 0; i < n; i++) {
        if (stk.isEmpty() == true) stk.push(arr[i]);
        else {
            // check if current element is greater than top of stack, push then
            if (arr[i] > stk.peek()) {
                smallerToLeft[i] = stk.peek();
                stk.push(arr[i]);
            }
            else {
                stk.pop();
                i--;   // why? there might be more elements on stack which are greater than arr[i]
            }
        }
    }
